...
const GoogleTag: React.FC<GoogleTagProps> = ({ formData, handleChange }) => {
    return (
       <><div className='GoogleTag mb-4 mt-4'>
       <div className='flex gap-2 items-center'>
               <div> <Image src={googleTag} alt="googleTag" width={40} className='h-auto' /></div>
               <div className='text-1xl md:text-2xl text-black'>Google Tag</div>
           </div>
           <p className='text-md text-gray-400 mb-2'>We require access to GTM so we can check the set up of the events and conversion tracking once the new website is up and running.
       ...
       <div className='radio-buttons flex items-center gap-2 mb-7'>
                <label className={`inline-flex items-center ${formData.googleTag === 'Completed' ? 'text-green-600' : ''}`}>
                    <div
                        className={`w-5 h-5 border rounded-md flex items-center justify-center mr-2 ${formData.googleTag === 'Completed' ? 'bg-green-600' : 'bg-white border-gray-400'}`}
                ...
                <label className={`inline-flex items-center ${formData.googleTag === 'Stuck' ? 'text-red-600' : ''}`}>
                    <div
                        className={`w-5 h-5 border rounded-md flex items-center justify-center mr-2 ${formData.googleTag === 'Stuck' ? 'bg-red-600' : 'bg-white border-gray-400'}`}

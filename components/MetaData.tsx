...
const MetaData: React.FC<MetaDataProps> = ({ formData, handleChange }) => {
    return (
        <>
            <div className='MetaData mb-4 mt-4'>
             ...
             <div className='radio-buttons flex items-center gap-2 mb-7'>
                <label className={`inline-flex items-center ${formData.metaStatus === 'Completed' ? 'text-green-600' : ''}`}>
                    <div
                        className={`w-5 h-5 border rounded-md flex items-center justify-center mr-2 ${formData.metaStatus === 'Completed' ? 'bg-green-600' : 'bg-white border-gray-400'}`}
                ...
                <label className={`inline-flex items-center ${formData.metaStatus === 'Stuck' ? 'text-red-600' : ''}`}>
                    <div
                        className={`w-5 h-5 border rounded-md flex items-center justify-center mr-2 ${formData.metaStatus === 'Stuck' ? 'bg-red-600' : 'bg-white border-gray-400'}`}
